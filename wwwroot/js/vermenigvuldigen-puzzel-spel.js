...
function nieuweVraag() {
  ...
  const opties = genereerOpties(juisteAntwoord, maxA * tafelVan);
...
function genereerOpties(juist, max) {
  ...
  while (set.size < 4 && pogingen < 300) {
    ...
    if (kandidaat >= 1 && !set.has(kandidaat)) set.add(kandidaat);
  }
  while (set.size < 4) {
    set.add(randomInt(Math.max(1, juist - 5), juist + 5));
  }
  return shuffle([...set]);
}
